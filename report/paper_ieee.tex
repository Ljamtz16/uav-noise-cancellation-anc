...
\usepackage{url}
\usepackage{hyperref}
% cite is built into IEEEtran; siunitx/algorithm replaced with macros below
...
\begin{table}[t]
  \centering
  \caption{Algorithm 1: LMS Adaptive Filter (sample-by-sample)}
  \label{alg:lms}
  \begin{tabular}{@{}ll@{}}
    \toprule
    \textbf{Step} & \textbf{Operation} \\
    \midrule
    Input  & Reference buffer $\bm{x}$, desired $d$, step $\mu$, order $L$ \\
    Init   & $\bm{w} \leftarrow \bm{0}_L$ \\
    For $n=1\ldots N$: & \\
    \quad Output    & $y(n) = \bm{w}^\top\bm{x}(n)$ \\
    \quad Error     & $e(n) = d(n) - y(n)$ \\
    \quad Update    & $\bm{w} \leftarrow \bm{w} + \mu\,e(n)\,\bm{x}(n)$ \\
    Output & Error signal $\bm{e}$, filtered output $\bm{y}$ \\
    \bottomrule
  \end{tabular}
\end{table}
